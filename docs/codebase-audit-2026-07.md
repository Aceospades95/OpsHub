...
| **Bid pipeline (2026-07-08 request)** | ✅ New permissioned **Bids** module: `BidPortal` registry (procurement sites we're registered on, with renewal tracking) + `BidOpportunity` pipeline (IDENTIFIED → PREPARING → SUBMITTED → WON / NOT AWARDED / NO BID / STALE) with automatic stage timestamps, pipeline/table views with per-stage dollar totals, due-date chips + daily `bid-due-check` notifications, "waiting Nd" staleness hints on submitted bids, and one-click convert-won-bid-to-project (links `projectId`, requires create rights on both modules). Portals live inside the module (not intranet) so source attribution and win rates fall out of the data. Details in `docs/entity-map.md` §"Bid pipeline". |
| **Hardening + cleanup batch (2026-07-09)** | ✅ AWS runbook in `docs/deployment.md` (image pulls via ECR+IAM or GHCR PAT, S3 storage cutover, SES/SMTP email, EventBridge/cron for the job endpoint, backup+restore drill, sizing) + ready-to-enable `ecr-publish.yml` (OIDC, gated on repo vars). **/admin/census**: live row counts + last-activity for every P5 deletion candidate, legacy-role counts, ProjectMember↔Assignment overlap, storage/email driver stats — the facts needed to run the deferred deletions and migrations safely. Dedup: shared `EntityContactSection` (clients + suppliers) and `EntityFileSection` (supplier receipts + bid attachments). Bids round-out: comments + @mentions, private file attachments, `bids-win-rate` report (per-portal win rate + won value), Bids card on client pages, "My open bids" strip on /my. |
| **Google Tasks: reachable + reliable (2026-07-09→10)** | ✅ Round 1: read-only "assigned"/Gmail-linked tasks pull cleanly and never 403 the push (flagged `sourceReadOnly`); the Gmail/Docs link rides along (`Task.sourceLink`); adjustable auto-sync (`autoSyncMinutes`, client polling); opt-in "also add to the assignee's Google Tasks" on task create (`pushNewTaskToGoogle`). Round 2: sync controls extracted to shared `GoogleSyncControls` and mounted on `/tasks` (not just /my); Google mark + "Email" chip on `/tasks` rows in both views and the task drawer; every row is a deep-link anchor — `/tasks#task-<id>` scrolls to the row and opens its drawer, and My View titles + assignment notifications link there. Engine fixes: push ownership is by **list**, not assignee (reassigning a synced task to another connected user used to make *their* sync patch a foreign list → 404 → silent soft-delete); `pushNewTaskToGoogle` now stores the real default-list id (the stored `@default` alias never matched pull keys, so pushed tasks re-imported as duplicates); pull recognizes + migrates alias keys; 404 delete-mirroring only fires when list ownership is certain. The /tasks New Task dialog gained the same push-to-Google opt-in as project/client pages. |

| **Architecture review batch (2026-07-11)** | ✅ Nine-part enhancement review — full analysis, designs, and phased roadmap in `docs/architecture-review-2026-07.md`. Shipped alongside it: Google Tasks one-time full pull backfills `sourceLink` on pre-existing rows (`fullPulledAt` + migration); quote editor document actions (Download/Preview/Print/Email without leaving the editor, auto-save-first w/ dirty tracking); **Email quote** connects the dormant send/track schema — tokenized public PDF route (`/api/public/quotes/[token]/pdf`), `quote-sent` template, DRAFT→SENT + `sentAt`, first-open → `firstViewedAt` + VIEWED + `downloaded` QuoteEvents; PDF redesigned (accent band, embedded logo bytes, meta grid, real table, tinted totals, per-page footer w/ page numbers) with a configurable **Document accent color** in /admin/theme (`branding.accentColor`); table-first defaults + per-module remembered view (cookie + `resolveViewPreference`) across 9 modules (bids keeps Pipeline); jobs framework gains **dry-run previews + self-explaining run ledgers** (cert + vehicle checks; `ctx.dryRun`, `supportsDryRun`, admin Preview button) answering "checked 9, sent 0"; recovery-bin purge is now a creatable scheduled task (reliability finding #1); placeholder Embed widget delisted (#4). Reliability audit: 16 findings documented with owners in the review doc. |

| **Phase B: configurability core (2026-07-11)** | ✅ Four workstreams (details in `docs/architecture-review-2026-07.md` §roadmap): **Notification engine** — `NotificationRule` per type (enable, channels, +recipients by role/user/external email, subject/body `{{var}}` overrides, per-entity throttle) consulted by every `notify()`; per-recipient email personalization; type registry + "Delivery rules" admin UI with test sends; dead types removed. **Jobs v2** — typed per-job settings (`JobConfig.params` + schema-driven form), 3-strike failure alerting to admins, cadence overrides made real for every-tick jobs. **Imports v2** — transaction-rollback dry-run preview, 4 modes incl. fill-blanks across all 20 importers (users/clients always-upsert bug fixed), warning status for silent FK drops/enum coercions, ImportLog failed/warnings + fixed pagination + results CSV, onboarding triggers fired from import/JIT, 3 new fleet importers. **Fleet v2** — per-service-type schedules (months+miles), driver Log-Maintenance flow w/ schedule re-arm + mileage, registration expiry, escalating job w/ tunable thresholds. **Reliability** — findings #2,#3,#5–#10,#12–#16 fixed. |

| **Phase C: work logs + preferences + builder QA (2026-07-12)** | ✅ **Work Logs module** replaces the Google Form + reminder sheet: daily per-person logs (upsert semantics kill double-counting), current+previous-week back-fill, PTO/sick/holiday ScheduleExceptions (org-wide holidays supported) that suppress reminders, roster windows (join/termination aware), overtime flagged vs approved (WorkWeekFlag), frozen Monday snapshots w/ live delta, exceptions-aware `work-log-reminders` job (dry-runnable ledger + Monday escalation), historical importer, weekly report. **Per-user notification mutes** per channel on /notifications, honored by the engine. **Fleet**: receipts/photos on vehicles (driver-uploadable, authz-covered), quick Update-mileage action, notes in list views. **Builder QA sweep**: report-preview wrapping fixed + one consistent table treatment across all preview/data tables; broken controls repaired (relation sorts erroring saved custom reports — runtime now emits Prisma's nested orderBy; widget filter values typed + server-coerced; status-board group-by blanking; enum multi-select filters; purge task label; theme hex paste; various stuck spinners/silent failures). Supplier editing verified complete. |
