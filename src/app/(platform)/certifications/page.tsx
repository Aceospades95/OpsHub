...
import { Award, AlertTriangle, Clock, CheckCircle2, XCircle, RotateCcw } from "lucide-react";
...
export default async function CertificationsPage() {
  ...
  const perms = await resolveModulePerms(session.user.id, session.user.role, "certifications");
  ...
  const canCreate = session.user.role === "ADMIN" || session.user.role === "MANAGER" || session.user.role === "DEVELOPER";
  ...
  return (
    <div>
          ...
          <CardContent>
            <div className="space-y-2">
              {expiringSoon.map((cert) => {
                ...
                return (
                  <Link key={cert.id} href={`/certifications/${cert.id}`} className="flex items-center gap-3 py-2 px-3 rounded-md hover:bg-muted transition-colors">
                    ...
                    {cert.renewalRequirements && (
                      <Badge variant="outline" className="text-[10px]">Renewal info</Badge>
                    )}
                  </Link>
                );
              })}
      ...
      {certifications.length === 0 ? (
        <EmptyState icon={Award} title="No certifications yet" description="Add your first certification to start tracking renewals" />
      ) : (
          ...
          {certifications.map((cert) => {
            ...
            return (
              <Link key={cert.id} href={`/certifications/${cert.id}`}>
                <Card className={`hover:shadow-md transition-shadow h-full ${isExpired ? "border-destructive/50" : isExpiring ? "border-warning/50" : ""}`}>
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-foreground text-sm truncate">{cert.name}</h3>
                        {cert.issuingBody && <p className="text-xs text-muted-foreground">{cert.issuingBody}</p>}
                      </div>
                    ...
                    <div className="flex items-center gap-2 mt-3 flex-wrap">
                      ...
                      {cert.certNumber && <span className="text-xs text-muted-foreground">#{cert.certNumber}</span>}
                    ...
                    <div className="flex items-center gap-3 mt-3 pt-3 border-t border-border text-xs">
                      {isExpired && (
                        <span className="flex items-center gap-1 text-destructive font-medium">
                          ...
                          {cert.expirationDate && ` ${format(cert.expirationDate, "MMM d, yyyy")}`}
                        </span>
                      )}
                      {isExpiring && (
